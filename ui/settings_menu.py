...
class settings_menu(ui_base):
    def __init__(self):
        ...
        # Temporary layout
        fullscreen_checkbox = DirectCheckButton(text="Fullscreen", pos=(0,0,0),scale=0.2, indicatorValue=get_fullscreen_value(), command=self.toggle_fullscreen, text_font=self.font, relief=None)
        self.ui_elements.append(fullscreen_checkbox)
       
        current_music_volume = get_music_volume() 
        music_slider_text = DirectLabel(text="Music volume", scale=0.1, pos=(-0.5,0,-0.25), text_font=self.font, relief=None)
        ...
        sfx_slider_text = DirectLabel(text="SFX volume", scale=0.1, pos=(0.5,0,-0.25), text_font=self.font, relief=None)
        self.ui_elements.append(sfx_slider_text)
        self.sfx_volume_slider = DirectSlider(pageSize=1, range=(0,100), pos=(0.5,0,-0.3),  scale=0.4, value=int(current_sfx_volume * 100), command=self.update_sfx_volume, relief=None)
        self.ui_elements.append(self.sfx_volume_slider)
        play_sample_sfx_button = DirectButton(text=("Play sample sound"), pos=(0.5,0,-0.4), scale=0.05, command=self.play_sample_sound, text_font=self.font, relief=None)
        self.ui_elements.append(play_sample_sfx_button)
        
        main_menu_button = DirectButton(text=("return to main menu"), pos=(0,0,-0.6), scale=0.1, command=self.return_to_main_menu, text_font=self.font, relief=None)
